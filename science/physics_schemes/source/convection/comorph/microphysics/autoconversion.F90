...
module autoconversion_mod

implicit none

contains

! Subroutine to do autoconversion of liquid-cloud to rain or
! ice-cloud to snow in BOTEMS (Back-Of-The-Envelope Microphysical Scheme)
subroutine autoconversion( n_points, nc, index_ic,                             &
                           delta_t, vert_len, wf_cond1,                        &
                           q_cond1, q_cond2,                                   &
                           l_diags, diags_cond1, diags_cond2,                  &
...
use moist_proc_diags_type_mod, only: diags_cond_type
...
! Number of points where autoconverted species is non-zero
integer, intent(in) :: nc
! Indices of those points
integer, intent(in) :: index_ic(n_points)
...
! Structures containing diagnostic flags etc.
type(diags_cond_type), intent(in) :: diags_cond1
type(diags_cond_type), intent(in) :: diags_cond2
...
if ( real(nc,real_cvprec) > indi_thresh * real(n_points,real_cvprec) ) then
  ...
  if ( autoc_opt==autoc_linear ) then
    ...
    do ic = 1, n_points
      ...
      b_quad = one + delta_t(ic) * wf_cond1(ic) / vert_len(ic)
      c_quad = - q_cond1(ic)
  ...
  do ic = 1, n_points
    ! Don't allow increment to reduce q_cl below min allowed value
    dq_auto(ic) = min( dq_auto(ic), q_cond1(ic) )
    ! Add increment to q_cl and q_rain
    q_cond1(ic) = q_cond1(ic) - dq_auto(ic)
    q_cond2(ic) = q_cond2(ic) + dq_auto(ic)
  end do

  if ( l_diags ) then
    ! Save autoconversion increment diagnostics, if requested
    if ( diags_cond1 % dq_aut % flag ) then
      ! Extract super-array address
      i_super = diags_cond1 % dq_aut % i_super
      ! Copy increment
      do ic = 1, n_points
        diags_super(ic,i_super) = diags_super(ic,i_super) - dq_auto(ic)
      end do
    end if
    if ( diags_cond2 % dq_aut % flag ) then
      ! Extract super-array address
      i_super = diags_cond2 % dq_aut % i_super
      ! Copy increment
      do ic = 1, n_points
        diags_super(ic,i_super) = diags_super(ic,i_super) + dq_auto(ic)
      end do
